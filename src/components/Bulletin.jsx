  ...
  return (
    <div className="bulletin-board p-4 md:p-10 h-screen md:h-auto w-full flex justify-center items-center flex-col">
      ...
      <div className="bulletin-posts grid grid-cols-1 w-full md:grid-cols-2 gap-4">
        {posts.length > 0 &&
          <div key={posts[0].id} className="bulletin-post h-10/12 card card-body p-4 border rounded shadow-lg bg-white hover:bg-slate-300 cursor-pointer">
            {posts[0].cover && (
        ...
        {posts.slice(1).map(post => (
          <div key={post.id} className="flex p-4 border rounded shadow-lg bg-white gap-3 hover:bg-slate-300 cursor-pointer">
            {post.cover && (
